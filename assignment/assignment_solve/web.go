	...
	if r.URL.Path == "/solve" {
		...
		if v, ok := r.Form["coef"]; ok {
			...
			if n, _ := fmt.Sscanf(v[0], "%v,%v,%v,%v,%v,%v,%v,%v,%v,%v,%v,%v", &a, &b, &c, &d, &e, &f, &g, &h, &i, &j, &k, &l); n == 12 {
				fmt.Fprintln(w, SolveEquation(a, b, c, d, e, f, g, h, i, j, k, l))
			} else {
				fmt.Fprintln(w, "incorrect number of coefficients must be 12")
			}
		} else {
			fmt.Fprintln(w, "incorrect parameter must be 'coef'")
		}
	} else {
...
func SolveEquation(a, b, c, d, e, f, g, h, i, j, k, l float64) string {
...
func GetSolution(m, mX, mY, mZ [][]float64) string {
	...
	if detm == 0 {
		...
		result = fmt.Sprintf("x = %.2v, y = %.2v, z = %.2v\n", X, Y, Z)
	}
	return result
}
